...
import { Dialog, DialogContent, DialogTitle, TextField, Button, Grid, Snackbar } from "@material-ui/core";
...
function SavedSurveyGetLinkDialog({ isOpen, onClose, surveyId }) {
  ...
  useEffect(() => {
    const getSurveyLink = async () => {
      const response = await fetch(`https://myreactapp.site/api/surveys/link/${surveyId}`, {
        method: "GET",
        credentials: "include",
      });
  ...
  const recreateSurveyLink = async () => {
    const response = await fetch(`https://myreactapp.site/api/surveys/link/recreate/${surveyId}`, {
      method: "GET",
      credentials: "include",
    });
    const linkText = await response.text();
